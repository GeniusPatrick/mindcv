...
from mindspore import ParameterTuple, SummaryRecord, Tensor, load_param_into_net, ops, save_checkpoint
from mindspore.train.callback import Callback

from .checkpoint_manager import CheckpointManager
from .utils import AllReduceSum
...
class StateMonitor(Callback):
    ...
    def __init__(
        ...
        if self.rank_id in [0, None]:
            ...
            self.best_ckpt_path = os.path.join(ckpt_dir, f"{model_name}_best.ckpt")
            self.log_file = os.path.join(ckpt_dir, "result.log")
            log_title = "\t".join(["Epoch", "TrainLoss", *metric_name, "TrainTime", "EvalTime", "TotalTime"])
    ...
    def on_train_step_end(self, run_context):
        ...
        if cur_batch % self.log_interval == 0 or cur_batch == num_batches or cur_batch == 1:
            ...
            if self.rank_id in [0, None]:
                wandb.log(
                    {"step": cur_step, "loss": loss.asnumpy(), "lr": lr.asnumpy(), "t_step": self.step_time_accum}
                )
            self.step_time_accum = 0
    ...
    def on_train_epoch_end(self, run_context):
        ...
        if self.rank_id in [0, None]:
            if (cur_epoch % self.ckpt_save_interval == 0) or (cur_epoch == num_epochs):
                ...
                ckpt_save_path = os.path.join(self.ckpt_dir, f"{self.model_name}-{cur_epoch}_{cur_batch}.ckpt")
                self._manager.save_ckpoint(
        ...
        _logger.info(f"Total time since last epoch: {total_time:.6f}s")
        _logger.info("-" * 80)
        if self.rank_id in [0, None]:
            log_line = "\t".join(
                [
            ...
            wandb.log(
                {
                    ...
                    "Acc@5": res[1],  # todo
                    "t_epoch": total_time,
                    "t_eta": (num_epochs - cur_epoch) * total_time,
                }
            )
    ...
    def on_train_end(self, run_context):
        ...
        if self.dataset_val is not None:
            _logger.info(f"The best validation {self.metric_name[0]} is: {self.best_res} at epoch {self.best_epoch}.")
        _logger.info("=" * 80)
...
class ValCallback(Callback):
    ...
    def on_eval_step_end(self, run_context):
        ...
        if cur_step % self.log_step_interval == 0 or cur_step == num_batches:
